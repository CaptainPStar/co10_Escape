        ...
        if (!_isExecuted && time > _surpriseTimeSec) then {
            ...
            if (call _condition) then {
				...
				if (_surpriseID == "DROPCHOPPER_I") then {
                    ...
                    _dropGroup = createGroup resistance;
                    _dropUnits = [];
                    
                    for [{_i = 0}, {_i < _noOfDropUnits}, {_i = _i + 1}] do {
                        _soldierType = drn_arr_Escape_InfantryTypes_Ind select floor (random count drn_arr_Escape_InfantryTypes_Ind);
                        _soldier = _dropGroup createUnit [_soldierType, [0,0,30], [], 0, "FORM"];
                        //_soldier setSkill (_minEnemySkill + random (_maxEnemySkill - _minEnemySkill));
						//[_soldier, drn_var_Escape_enemyMinSkill] call EGG_EVO_skill;
                        _soldier setRank "CAPTAIN";
				...
				if (_surpriseID == "SEARCHDRONE") then {
					...
					_arr = [(getMarkerPos "drn_russianSearchChopperStartPosMarker"), 0, (a3e_arr_searchdrone select floor (random count a3e_arr_searchdrone)), RESISTANCE] call bis_fnc_spawnvehicle;
