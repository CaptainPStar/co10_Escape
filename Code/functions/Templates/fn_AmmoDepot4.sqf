...
_fnc_createObject = {
    ...
    if (_car != "") then {
		...
		_pos = [_center,_center vectorAdd [-7.52502,-19.5674,4.76837],_rotation] call _fnc_rotatePos;
    ...
    if (count _staticWeaponClasses > 0) then {
		...
		_pos = [_center,_center vectorAdd [-14.3452,-5.09326,-0.00143909],_rotation] call _fnc_rotatePos;
    ...
    if (count _staticWeaponClasses > 0) then {
		...
		_pos = [_center,_center vectorAdd [15.0289,10.7007,-0.00143814],_rotation] call _fnc_rotatePos;
		_obj = createVehicle [_gun, _pos, [], 0, "NONE"];
		_obj setVectorDirAndUp [[-0.999998,0.00176479,0],[0,0,1]];
		_obj setdir ((getdir _obj) + _rotation);
		_obj setPosATL _pos;
...
/*		_pos = [_center,_center vectorAdd [-4.68042,-5.16992,0],_rotation] call _fnc_rotatePos;
...
/*		_pos = [_center,_center vectorAdd [-4.46521,-0.0180664,0],_rotation] call _fnc_rotatePos;
...
/*		_pos = [_center,_center vectorAdd [-5.61267,-2.32764,0],_rotation] call _fnc_rotatePos;
...
*/		
    ...
    if (count _weapons > 0 || count _weaponMagazines > 0 || count _items > 0) then {
		...
		/*		
		_pos = [_center,_center vectorAdd [-7.30652,-4.67676,0],_rotation] call _fnc_rotatePos;
...
/*		_pos = [_center,_center vectorAdd [-7.4679,-0.294922,0],_rotation] call _fnc_rotatePos;
		_box = "Box_East_Wps_F" createvehicle _pos;
...
/*		_pos = [_center,_center vectorAdd [-5.82874,2.37256,0],_rotation] call _fnc_rotatePos;
		_box = "Box_East_WpsLaunch_F" createvehicle _pos;
